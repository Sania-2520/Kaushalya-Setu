...
'use server';
/**
 * @fileOverview A Genkit flow to analyze a resume PDF and provide feedback.
 *
...
const resumeFeedbackPrompt = ai.definePrompt({
  ...
  model: 'googleai/gemini-2.0-flash', // Assuming this model supports PDF media input
...
7.  **Skills Section:** Are relevant technical skills (e.g., programming languages, tools) and soft skills listed? Is there a good balance? Are they too generic or specific enough? (e.g., "Categorize your skills into Technical and Soft Skills for better clarity.")
...
10. **Specific Actionable Suggestions:** Provide 3-5 concrete, numbered suggestions for improvement. For example:
...
const resumeFeedbackFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    if (!output) {
        throw new Error("No feedback was generated by the AI.");
    }
    return output;
  }
);
